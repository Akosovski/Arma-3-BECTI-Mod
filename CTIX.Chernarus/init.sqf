...
0 setFog [0, 0, 0]; // Clears fog at the start of the mission
...
if (CTI_IsClient && !CTI_IsHeadless) then {
	...
	if(isClass(configFile >> "cfgPatches" >> "task_force_radio"))then{
		[cti_ammobox_east,["tf_bussole"],true] call BIS_fnc_addVirtualBackPackCargo;
		[cti_ammobox_west,["tf_rt1523g_rhs"],true] call BIS_fnc_addVirtualBackPackCargo;
	...
	FAR_ReviveMode = 1;
	call compileFinal preprocessFileLineNumbers "Client\FAR_revive\FAR_revive_init.sqf";
};
