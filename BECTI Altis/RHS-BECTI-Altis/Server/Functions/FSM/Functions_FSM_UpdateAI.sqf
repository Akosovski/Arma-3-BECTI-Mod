...
CTI_FSM_UpdateAI_Order_Move = {
	...
	for "_i" from 0 to (count waypoints _group) do {deleteWaypoint [_group,_i]};
...
CTI_FSM_UpdateAI_Order_TakeTowns = {
	...
	if !(isNull _assigned_target) then { //--- Assign a target to the AI if found and swap the order to the town one.
		...
		if !(unitReady leader _group) then {for "_i" from 0 to (count waypoints _group) do {deleteWaypoint [_group,_i]};leader _group addWaypoint [(getPos leader _group),10]};
	};
};
...
CTI_FSM_UpdateAI_Order_TakeTown = {
	...
	if( (_group getVariable "cti_role") in (missionNamespace getVariable format["CTI_SQUADS_%1_KIND_INFANTRY",_side]) )then
	{
		/*_has_truck=false;
		{
			if( (vehicle _x) isKindOf "truck_f" OR )
		};*/
		_units = units _group;
		_vehicles=[];
		if( !(isNull(_group getVariable ["rhs_grp_truck",objNull])) )then{_vehicles pushBack _group getVariable "rhs_grp_truck";diag_log format["powrot do ciezarowki %1",_group]};
		_interacts = [];
		{
			_roles = assignedVehicleRole _x;
			if (count _roles > 0) then { //--- It's assigned
			...
			_has_vehicle = false;
			{
				if (local _x && isNil{_x getVariable "cti_ai_prohib"} && canMove _x && !(_x in _units) && alive driver _x && _x emptyPositions "cargo" > 0) then { //--- Vehicle is local to our server, it's usable, it can move
					...
					if (!isNil '_remote_goto' && side _group == side _x) then {
						_cargo = abs ((_x emptyPositions "cargo") - count(assignedCargo _x)); //--- Get the vehicle absolute cargo
						if (_remote_goto distance _x > (CTI_AI_TEAMS_CARGO_VEHICLES_DISEMBARK_RANGE+50) && _local_goto distance _remote_goto <= CTI_AI_TEAMS_CARGO_VEHICLES_MATCH_RANGE && _unit distance _local_goto > (CTI_AI_TEAMS_CARGO_VEHICLES_DISEMBARK_RANGE+50) && _cargo > 0) then { //--- Objectives aligns?
							if(_unit distance _x < 450)then{_has_vehicle = true; unassignVehicle _unit; _unit assignAsCargo _x; [_unit] orderGetIn true};
						};
						 //player sidechat format ["%1 %2",_x ,_has_vehicle];
					};
				};
	...
	/*if( !(isNull(_group getVariable ["rhs_grp_truck",objNull])) )then
	{


		//(leader _group) assignAsDriver (_group getVariable "rhs_grp_truck");
		//[(leader _group)] orderGetIn true;
		_wp = _group addWaypoint [(_group getVariable "rhs_grp_truck")];
		_wp setWaypointType "GETIN";
		diag_log "wsiadamy";
		player globalChat format["wsiadamy do pojazdu: %1 %2 %3",_group,_group getVariable "rhs_grp_truck",_town];
	};*/
	_wppos=([([getPos _town, 5, 40] call CTI_CO_FNC_GetRandomPosition),20] call CTI_CO_FNC_GetEmptyPosition);

	_wp = _group addWaypoint [_wppos,50];
	...
	if (_side_owned && _order in [CTI_ORDER_TAKEHOLDTOWN_AUTO, CTI_ORDER_TAKEHOLDTOWN]) then {
		if (_town distance leader _group < CTI_AI_ORDER_TAKEHOLDTOWNS_RANGE) then {
			...
			while {true} do {
				...
				switch (_action) do {
					case "patrol": {if (_move_patrol_reload || unitReady leader _group && random 100 > (45 + (random 15))) then {if (_move_patrol_reload) then {_move_patrol_reload = false};_move_to = _patrol_area select floor(random count _patrol_area);for "_i" from 0 to (count waypoints _group) do {deleteWaypoint [_group,_i]}; _group addWaypoint [_move_to,30]}};
					case "defense": {if (unitReady leader _group && time - _move_defend_last > 65) then {_move_defend_last = time; for "_i" from 0 to (count waypoints _group) do {deleteWaypoint [_group,_i]};_group addWaypoint [([_town, 10, 50] call CTI_CO_FNC_GetRandomPosition),50]}};
				};
...
//todo: improve next node picking (distance > max / 2 or so).
	...
	while {true} do {
		...
		switch (_action) do {
			case "patrol": {if (_move_patrol_reload || unitReady leader _group && random 100 > (45 + (random 15))) then {if (_move_patrol_reload) then {_move_patrol_reload = false};_move_to = _patrol_area select floor(random count _patrol_area);for "_i" from 0 to (count waypoints _group) do {deleteWaypoint [_group,_i]}; _group addWaypoint [_move_to,10]}};
			case "defense": {if (unitReady leader _group && time - _move_defend_last > 65) then {_move_defend_last = time; for "_i" from 0 to (count waypoints _group) do {deleteWaypoint [_group,_i]};_group addWaypoint [([_defend, 10, 50] call CTI_CO_FNC_GetRandomPosition),20]}};
		};
