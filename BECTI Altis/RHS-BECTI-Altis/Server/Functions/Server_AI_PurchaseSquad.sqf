...
private ["_can_use", "_compose", "_factory", "_flaten", "_group", "_need", "_picked", "_pool", "_probability", "_side","_truck","_buildTruck"];
...
diag_log format["możliwoci %2: %1",_pool,_group];

if( (_group getVariable "cti_role") in (missionNamespace getVariable format["CTI_SQUADS_%1_KIND_INFANTRY",_side]) )then
{
	_truck = (_pool select 0) select 0;
	_buildTruck=true;
	{
		if( (vehicle _x) isKindOf "truck_f")then{_buildTruck=false};
	}foreach units _group;

	if( _buildTruck ) then{
		diag_log "budujemy cieżarówke";
		_seed = time + random 10000 - random 500 + diag_frameno;
		[_group, _group, _side, _truck, _factory, [true, true, true, true, true], _seed] call CTI_SE_FNC_OnClientPurchase;
	};
};
...
//{_compose pushBack (_x select 0)}foreach _flaten;
...
diag_log format["do stworzenia: %1",_compose];
{
	_seed = time + random 10000 - random 500 + diag_frameno;
